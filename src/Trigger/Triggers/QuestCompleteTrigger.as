package Trigger.Triggers
{
    ...
    public final class QuestCompleteTrigger extends InstantTrigger implements Observer 
    {

        private var waitingForQuest:dQuestElementVO;

        public function QuestCompleteTrigger(_arg_1:Triggerable, _arg_2:TriggerVO, _arg_3:cGeneralInterface)
        {
        ...
        {
            if (((!(this.waitingForQuest == null)) && (this.waitingForQuest.mQuestMode >= QuestManagerStatic.QUEST_MODE_DEACTIVATED)))
            {
                if (this.waitingForQuest.mQuestMode == QuestManagerStatic.QUEST_MODE_DEACTIVATED)
                {
                    (para as dQuestPoolVO).addPropertyObserver(dQuestPoolVO.POOL_NOTIFICATION_string, this);
